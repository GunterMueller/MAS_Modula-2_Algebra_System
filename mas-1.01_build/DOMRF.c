#include "SYSTEM_.h"

#ifndef DEFINITION_MASSTOR
#include "MASSTOR.h"
#endif

#ifndef DEFINITION_MASERR
#include "MASERR.h"
#endif

#ifndef DEFINITION_MASADOM
#include "MASADOM.h"
#endif

#ifndef DEFINITION_MASBIOS
#include "MASBIOS.h"
#endif

#ifndef DEFINITION_SACLIST
#include "SACLIST.h"
#endif

#ifndef DEFINITION_DIPRF
#include "DIPRF.h"
#endif

#ifndef DEFINITION_SACPOL
#include "SACPOL.h"
#endif

#ifndef DEFINITION_DIPAGB
#include "DIPAGB.h"
#endif

#ifndef DEFINITION_DIPGB
#include "DIPGB.h"
#endif

#ifndef DEFINITION_DOMRF
#include "DOMRF.h"
#endif

MASADOM_Domain DOMRF_DOMRFD;
CHAR DOMRF_rcsid [] = "$Id: DOMRF.md,v 1.2 1992/02/12 17:31:33 pesch Exp $";
CHAR DOMRF_copyright [] = "Copyright (c) 1989 - 1992 Universitaet Passau";

static CHAR rcsidi [] = "$Id: DOMRF.mi,v 1.5 1994/09/06 11:49:11 rose Exp $";
static CHAR copyrighti [] = "Copyright (c) 1989 - 1992 Universitaet Passau";
static MASSTOR_LIST DDIF ARGS((MASSTOR_LIST C_75_A, MASSTOR_LIST B));
static MASSTOR_LIST DEXP ARGS((MASSTOR_LIST C_74_A, MASSTOR_LIST NL));
static MASSTOR_LIST DFI ARGS((MASSTOR_LIST D, MASSTOR_LIST C_73_A));
static MASSTOR_LIST DFIP ARGS((MASSTOR_LIST D, MASSTOR_LIST C_72_A));
static MASSTOR_LIST DINV ARGS((MASSTOR_LIST C_71_A));
static MASSTOR_LIST DINVT ARGS((MASSTOR_LIST C_70_A));
static MASSTOR_LIST DNEG ARGS((MASSTOR_LIST C_69_A));
static MASSTOR_LIST DONE ARGS((MASSTOR_LIST C_68_A));
static MASSTOR_LIST DPNF ARGS((MASSTOR_LIST G, MASSTOR_LIST P));
static MASSTOR_LIST DPROD ARGS((MASSTOR_LIST C_67_A, MASSTOR_LIST B));
static MASSTOR_LIST DPSP ARGS((MASSTOR_LIST C_66_A, MASSTOR_LIST B));
static MASSTOR_LIST DPSUGNF ARGS((MASSTOR_LIST G, MASSTOR_LIST P));
static MASSTOR_LIST DPSUGSP ARGS((MASSTOR_LIST C_65_A, MASSTOR_LIST B));
static MASSTOR_LIST DQUOT ARGS((MASSTOR_LIST C_64_A, MASSTOR_LIST B));
static MASSTOR_LIST DREAD ARGS((MASSTOR_LIST D));
static MASSTOR_LIST DSIGN ARGS((MASSTOR_LIST C_63_A));
static MASSTOR_LIST DSUM ARGS((MASSTOR_LIST C_62_A, MASSTOR_LIST B));
static void DWRIT ARGS((MASSTOR_LIST C_61_A));
static MASSTOR_LIST DDDRD ARGS(());
static void DDDWR ARGS((MASSTOR_LIST D));


static MASSTOR_LIST DDIF
# ifdef __STDC__
(MASSTOR_LIST C_75_A, MASSTOR_LIST B)
# else
(C_75_A, B)
MASSTOR_LIST C_75_A, B;
# endif
{
  MASSTOR_LIST AL, AP, BL, BP, C, CL;

  MASSTOR_ADV(C_75_A, &AL, &AP);
  MASSTOR_ADV(B, &BL, &BP);
  CL = DIPRF_RFDIF(AL, BL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DEXP
# ifdef __STDC__
(MASSTOR_LIST C_74_A, MASSTOR_LIST NL)
# else
(C_74_A, NL)
MASSTOR_LIST C_74_A, NL;
# endif
{
  MASSTOR_LIST AL, AP, C, CL;

  MASSTOR_ADV(C_74_A, &AL, &AP);
  CL = DIPRF_RFEXP(AL, NL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DFI
# ifdef __STDC__
(MASSTOR_LIST D, MASSTOR_LIST C_73_A)
# else
(D, C_73_A)
MASSTOR_LIST D, C_73_A;
# endif
{
  MASSTOR_LIST C, CL, RL, BL;

  MASSTOR_ADV(D, &BL, &D);
  RL = DIPRF_RFNOV(BL);
  CL = SACPOL_PINV(0, C_73_A, RL);
  CL = DIPRF_RFFIP(RL, CL);
  C = MASSTOR_COMP(CL, D);
  return C;
}

static MASSTOR_LIST DFIP
# ifdef __STDC__
(MASSTOR_LIST D, MASSTOR_LIST C_72_A)
# else
(D, C_72_A)
MASSTOR_LIST D, C_72_A;
# endif
{
  MASSTOR_LIST C, CL, RL, BL;

  MASSTOR_ADV(D, &BL, &D);
  RL = DIPRF_RFNOV(BL);
  CL = DIPRF_RFFIP(RL, C_72_A);
  C = MASSTOR_COMP(CL, D);
  return C;
}

static MASSTOR_LIST DINV
# ifdef __STDC__
(MASSTOR_LIST C_71_A)
# else
(C_71_A)
MASSTOR_LIST C_71_A;
# endif
{
  MASSTOR_LIST AL, AP, C, CL;

  MASSTOR_ADV(C_71_A, &AL, &AP);
  CL = DIPRF_RFINV(AL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DINVT
# ifdef __STDC__
(MASSTOR_LIST C_70_A)
# else
(C_70_A)
MASSTOR_LIST C_70_A;
# endif
{
  MASSTOR_LIST AL, AP, TL;

  MASSTOR_ADV(C_70_A, &AL, &AP);
  TL = 1;
  if (DIPRF_RFNUM(AL) == 0) {
    TL = 0;
  }
  return TL;
}

static MASSTOR_LIST DNEG
# ifdef __STDC__
(MASSTOR_LIST C_69_A)
# else
(C_69_A)
MASSTOR_LIST C_69_A;
# endif
{
  MASSTOR_LIST AL, AP, C, CL;

  MASSTOR_ADV(C_69_A, &AL, &AP);
  CL = DIPRF_RFNEG(AL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DONE
# ifdef __STDC__
(MASSTOR_LIST C_68_A)
# else
(C_68_A)
MASSTOR_LIST C_68_A;
# endif
{
  MASSTOR_LIST AL, AP, SL;

  MASSTOR_ADV(C_68_A, &AL, &AP);
  SL = DIPRF_RFONE(AL);
  return SL;
}

static MASSTOR_LIST DPNF
# ifdef __STDC__
(MASSTOR_LIST G, MASSTOR_LIST P)
# else
(G, P)
MASSTOR_LIST G, P;
# endif
{
  return DIPGB_DIPNOR(G, P);
}

static MASSTOR_LIST DPROD
# ifdef __STDC__
(MASSTOR_LIST C_67_A, MASSTOR_LIST B)
# else
(C_67_A, B)
MASSTOR_LIST C_67_A, B;
# endif
{
  MASSTOR_LIST AL, AP, BL, BP, C, CL;

  MASSTOR_ADV(C_67_A, &AL, &AP);
  MASSTOR_ADV(B, &BL, &BP);
  CL = DIPRF_RFPROD(AL, BL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DPSP
# ifdef __STDC__
(MASSTOR_LIST C_66_A, MASSTOR_LIST B)
# else
(C_66_A, B)
MASSTOR_LIST C_66_A, B;
# endif
{
  return DIPGB_DIPSP(C_66_A, B);
}

static MASSTOR_LIST DPSUGNF
# ifdef __STDC__
(MASSTOR_LIST G, MASSTOR_LIST P)
# else
(G, P)
MASSTOR_LIST G, P;
# endif
{
  return DIPAGB_EDIPSUGNOR(G, P);
}

static MASSTOR_LIST DPSUGSP
# ifdef __STDC__
(MASSTOR_LIST C_65_A, MASSTOR_LIST B)
# else
(C_65_A, B)
MASSTOR_LIST C_65_A, B;
# endif
{
  return DIPAGB_EDIPSUGSP(C_65_A, B);
}

static MASSTOR_LIST DQUOT
# ifdef __STDC__
(MASSTOR_LIST C_64_A, MASSTOR_LIST B)
# else
(C_64_A, B)
MASSTOR_LIST C_64_A, B;
# endif
{
  MASSTOR_LIST AL, AP, BL, BP, C, CL;

  MASSTOR_ADV(C_64_A, &AL, &AP);
  MASSTOR_ADV(B, &BL, &BP);
  CL = DIPRF_RFQ(AL, BL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static MASSTOR_LIST DREAD
# ifdef __STDC__
(MASSTOR_LIST D)
# else
(D)
MASSTOR_LIST D;
# endif
{
  MASSTOR_LIST C, CL, V;

  D = MASSTOR_RED(D);
  V = MASSTOR_FIRST(D);
  CL = DIPRF_RFREAD(V);
  C = MASSTOR_COMP(CL, D);
  return C;
}

static MASSTOR_LIST DSIGN
# ifdef __STDC__
(MASSTOR_LIST C_63_A)
# else
(C_63_A)
MASSTOR_LIST C_63_A;
# endif
{
  MASSTOR_LIST AL, SL;

  AL = MASSTOR_FIRST(C_63_A);
  SL = DIPRF_RFSIGN(AL);
  return SL;
}

static MASSTOR_LIST DSUM
# ifdef __STDC__
(MASSTOR_LIST C_62_A, MASSTOR_LIST B)
# else
(C_62_A, B)
MASSTOR_LIST C_62_A, B;
# endif
{
  MASSTOR_LIST AL, AP, BL, BP, C, CL;

  MASSTOR_ADV(C_62_A, &AL, &AP);
  MASSTOR_ADV(B, &BL, &BP);
  CL = DIPRF_RFSUM(AL, BL);
  C = MASSTOR_COMP(CL, AP);
  return C;
}

static void DWRIT
# ifdef __STDC__
(MASSTOR_LIST C_61_A)
# else
(C_61_A)
MASSTOR_LIST C_61_A;
# endif
{
  MASSTOR_LIST AL, V;

  SACLIST_FIRST2(C_61_A, &AL, &V);
  DIPRF_RFWRIT(AL, V);
  return;
}

static MASSTOR_LIST DDDRD
# ifdef __STDC__
()
# else
()
# endif
{
  MASSTOR_LIST V, D, RL, AL;

  V = SACPOL_VLREAD();
  RL = MASSTOR_LENGTH(V);
  AL = DIPRF_RFFIP(RL, 0);
  D = SACLIST_LIST2(AL, V);
  return D;
}

static void DDDWR
# ifdef __STDC__
(MASSTOR_LIST D)
# else
(D)
MASSTOR_LIST D;
# endif
{
  MASSTOR_LIST V;

  D = MASSTOR_RED(D);
  V = MASSTOR_FIRST(D);
  SACPOL_VLWRIT(V);
  return;
}

void DOMRF_DomLoadRF
# ifdef __STDC__
()
# else
()
# endif
{
  MASADOM_Domain d;

  d = MASADOM_NewDom("RF", 2L, "Rational Function", 17L);
  DOMRF_DOMRFD = d;
  MASADOM_SetDifFunc(d, DDIF);
  MASADOM_SetExpFunc(d, DEXP);
  MASADOM_SetFIntFunc(d, DFI);
  MASADOM_SetFIPolFunc(d, DFIP);
  MASADOM_SetInvFunc(d, DINV);
  MASADOM_SetInvTFunc(d, DINVT);
  MASADOM_SetNegFunc(d, DNEG);
  MASADOM_SetOneFunc(d, DONE);
  MASADOM_SetProdFunc(d, DPROD);
  MASADOM_SetQuotFunc(d, DQUOT);
  MASADOM_SetReadFunc(d, DREAD);
  MASADOM_SetSignFunc(d, DSIGN);
  MASADOM_SetSumFunc(d, DSUM);
  MASADOM_SetWritFunc(d, DWRIT);
  MASADOM_SetDdrdFunc(d, DDDRD);
  MASADOM_SetDdwrFunc(d, DDDWR);
  MASADOM_SetPNormFunc(d, DPNF);
  MASADOM_SetPSpolFunc(d, DPSP);
  MASADOM_SetPSugNormFunc(d, DPSUGNF);
  MASADOM_SetPSugSpolFunc(d, DPSUGSP);
}

void BEGIN_DOMRF()
{
  static BOOLEAN has_been_called = FALSE;

  if (!has_been_called) {
    has_been_called = TRUE;

    BEGIN_MASADOM();
    BEGIN_MASSTOR();
    BEGIN_MASERR();
    BEGIN_MASADOM();
    BEGIN_MASBIOS();
    BEGIN_SACLIST();
    BEGIN_DIPRF();
    BEGIN_SACPOL();
    BEGIN_DIPAGB();
    BEGIN_DIPGB();

  }
}
