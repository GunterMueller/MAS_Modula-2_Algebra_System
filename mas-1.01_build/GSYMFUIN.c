...
CHAR GSYMFUIN_rcsid [] = "$Id: GSYMFUIN.md,v 1.1 1995/11/05 15:57:24 pesch Exp $";
CHAR GSYMFUIN_copyright [] = "Copyright (c) 1994 Universitaet Passau";

static CHAR rcsidi [] = "$Id: GSYMFUIN.mi,v 1.1 1995/11/05 15:57:27 pesch Exp $";
...
{
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("G-Symmetric Polynomial System Package:", 38L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("--------------------------------------", 38L);
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("              PG := GSYPGR(Number_of_Variables).", 48L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("    Symmetric_PG := GSYSPG(Number_of_Variables).", 48L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("           Order := GSYORD(PG).", 31L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GSYNSP(PG).", 11L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GSYPGW(PG).", 11L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GSYINF().", 9L);
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("           Orbit := GINORP(PG, Monom).", 38L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("      Orbit_List := GINOPL(PG, Monom_List).", 43L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("         Polynom := GINCHK(PG, Base, Base_Polynom).", 51L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("        Base_HTL := GINBAS(PG).", 31L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GINCUT(PG, Polynom, PG_Symmetric_Polynom, Remainder_Polynom).", 61L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GINRED(PG, Polynom, Base, Base_Polynom, Remainder_Polynom).", 59L);
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("           Orbit := GRNORP(PG, Monom).", 38L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("      Orbit_List := GRNOPL(PG, Monom_List).", 43L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("         Polynom := GRNCHK(PG, Base, Base_Polynom).", 51L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("        Base_HTL := GRNBAS(PG).", 31L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("        Base_HTL := GRNGGB(PG).", 31L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GRNCUT(PG, Polynom, PG_Symmetric_Polynom, Remainder_Polynom).", 61L);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GRNRED(PG, Polynom, Base, Base_Polynom, Remainder_Polynom).", 59L);
  MASBIOS_BLINES(1);
}

MASSTOR_LIST GSYMFUIN_GSYPGR
# ifdef __STDC__
(MASELEM_GAMMAINT N)
# else
(N)
  ...
  while (X != MASSTOR_SIL) {
    if (MASSTOR_LENGTH(X) != N) {
      ...
      for (;;) {
        ...
        if (SACLIST_MEMBER(I, X) == 0) {
          goto EXIT_1;
        }
        I = I + 1;
      } EXIT_1:;
      if (I > N) {
        PG = MASSTOR_COMP(X, PG);
      } else {
        MASBIOS_SWRITE("No permutation, try again! ", 27L);
      }
    }
    X = SACLIST_LREAD();
  }
...
# endif
{
  ...
  F1 = MASSTOR_SIL;
  for (I = N; I >= 1; I += -1) {
    F1 = MASSTOR_COMP(I, F1);
  }
  while (PG != MASSTOR_SIL) {
    MASSTOR_ADV(PG, &F2, &PG);
    MASBIOS_BLINES(0);
    SACLIST_OWRITE(F1);
    MASBIOS_SWRITE(" --> ", 5L);
    SACLIST_OWRITE(F2);
...
# endif
{
  ...
  while (HTL != MASSTOR_SIL) {
    ...
    while (NPG != MASSTOR_SIL) {
      ...
      NRES = DIPI_DIIPSM(RES, DIPC_DIPFMO(HK, TERM));
      if (NRES != 0) {
...
MASELEM_GAMMAINT GSYMFUIN_GSYORD
# ifdef __STDC__
(MASSTOR_LIST PG)
# else
(PG)
...
# endif
{
  MASSTOR_LIST HK, HT, POL;
  MASELEM_GAMMAINT ORDER, I, N;
  ...
  POL = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, HT));
  if (POL == 0) {
    POL = MASSTOR_SIL;
  }
...
# endif
{
  ...
  for (;;) {
    ...
    ORBIT_SPG = GSYMFUIN_GINORP(SPG, DIPC_DIPFMO(1, TERM));
    while (ORBIT_SPG != 0) {
      NR = NR + 1;
      ORBIT_PG = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, MASSTOR_FIRST(ORBIT_SPG)));
      ORBIT_SPG = DIPI_DIIPDF(ORBIT_SPG, ORBIT_PG);
    }
  } EXIT_2:;
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("There are ", 10L);
  SACLIST_OWRITE(NR);
  MASBIOS_SWRITE(" special polynomial(s).", 23L);
}

MASSTOR_LIST GSYMFUIN_GSYSPG
# ifdef __STDC__
(MASELEM_GAMMAINT N)
# else
(N)
...
# endif
{
  MASSTOR_LIST PG, PERM;
  MASELEM_GAMMAINT I, K;
    ...
    if (B_7 <= B_8)
      for (I = B_7;; I += 1) {
        PERM = MASSTOR_SIL;
        {
          ...
          if (B_9 <= B_10)
            for (K = B_9;; K += 1) {
              PERM = SACLIST_LEINST(PERM, K - 1, K);
              if (K >= B_10) break;
            }
        }
        K = I + 1;
        ...
        SACLIST_SLELT(PERM, K, I);
        SACLIST_SLELT(PERM, I, K);
        PG = SACLIST_LEINST(PG, I - 1, PERM);
        if (I >= B_8) break;
      }
  }
  return PG;
...
# endif
{
  ...
  while (ML != MASSTOR_SIL) {
    ...
    RES = MASSTOR_COMP(GSYMFUIN_GINORP(PG, HM), RES);
  }
  return MASSTOR_INV(RES);
}

void GSYMFUIN_GINCUT
# ifdef __STDC__
...
# endif
{
  ...
  *POL2 = 0;
  while (POL != 0) {
    ...
    ORBIT = GSYMFUIN_GINORP(PG, HM);
    if (SACLIST_EQUAL(HT, MASSTOR_FIRST(ORBIT)) == 1) {
      *POL1 = DIPI_DIIPSM(*POL1, ORBIT);
      POL = DIPI_DIIPDF(POL, ORBIT);
    } else {
      *POL2 = DIPI_DIIPSM(*POL2, HM);
      POL = DIPI_DIIPDF(POL, HM);
    }
  }
}

MASSTOR_LIST GSYMFUIN_GINCHK
# ifdef __STDC__
...
# endif
{
  ...
  BASE_ORBIT = GSYMFUIN_GINOPL(PG, BASE);
  while (POL != 0) {
    DIPC_DIPMAD(POL, &HK, &HT, &POL);
    if (POL == MASSTOR_SIL) {
      POL = 0;
    }
    PROD = DIPI_DIIPEX(SACLIST_LELT(BASE_ORBIT, 1), 0);
    {
      LONGINT B_11 = 1, B_12 = N;

      if (B_11 <= B_12)
        for (I = B_11;; I += 1) {
          PROD = DIPI_DIIPPR(PROD, DIPI_DIIPEX(SACLIST_LELT(BASE_ORBIT, I), SACLIST_LELT(HT, I)));
          if (I >= B_12) break;
        }
    }
    RES = DIPI_DIIPSM(RES, DIPI_DIIPIP(PROD, HK));
  }
  return RES;
}

void GSYMFUIN_GINCHKBAS
# ifdef __STDC__
...
# endif
{
  ...
  *POL = 0;
  ...
  while (NPOL != MASSTOR_SIL) {
    DIPC_DIPMAD(NPOL, &HK, &HT, &NPOL);
    {
      LONGINT B_17 = 1, B_18 = MASSTOR_LENGTH(POS);
    ...
    *POL = DIPI_DIIPSM(*POL, DIPC_DIPFMO(HK, HT));
  }
}

MASSTOR_LIST GSYMFUIN_GSYMLT
# ifdef __STDC__
(MASELEM_GAMMAINT N)
# else
(N)
...
# endif
{
  MASSTOR_LIST TL, SIGMA, SPG, ORBIT, HK, HT;
  MASELEM_GAMMAINT I, J;
  ...
  SPG = GSYMFUIN_GSYSPG(N);
  {
    LONGINT B_19 = 1, B_20 = N;

    if (B_19 <= B_20)
      for (I = B_19;; I += 1) {
        SIGMA = MASSTOR_SIL;
        {
          LONGINT B_21 = 1, B_22 = I;

          if (B_21 <= B_22)
            for (J = B_21;; J += 1) {
              SIGMA = MASSTOR_COMP(1, SIGMA);
              if (J >= B_22) break;
            }
        }
        {
          LONGINT B_23 = I + 1, B_24 = N;

          if (B_23 <= B_24)
            for (J = B_23;; J += 1) {
              SIGMA = MASSTOR_COMP(0, SIGMA);
              if (J >= B_24) break;
            }
        }
        ORBIT = GSYMFUIN_GINORP(SPG, DIPC_DIPFMO(1, SIGMA));
        while (ORBIT != MASSTOR_SIL) {
          DIPC_DIPMAD(ORBIT, &HK, &HT, &ORBIT);
          TL = MASSTOR_COMP(HT, TL);
        }
        if (I >= B_20) break;
      }
  }
  return MASSTOR_INV(TL);
}

MASELEM_GAMMAINT GSYMFUIN_GSYTWG
# ifdef __STDC__
(MASSTOR_LIST TERM1, MASSTOR_LIST TERM2)
# else
(TERM1, TERM2)
MASSTOR_LIST TERM1, TERM2;
# endif
{
  MASSTOR_LIST S_TERM1, S_TERM2;
  MASELEM_GAMMAINT SIGN;

  S_TERM1 = SACLIST_CINV(TERM1);
  S_TERM2 = SACLIST_CINV(TERM2);
  SACSET_LBIBS(S_TERM1);
  SACSET_LBIBS(S_TERM2);
  SIGN = DIPC_EVILCI(S_TERM1, S_TERM2);
  if (SIGN == 1) {
    return 1;
  }
  if (SIGN == 0) {
    return DIPC_EVCOMP(TERM1, TERM2);
  }
  return -1;
}
...
# endif
{
  ...
  *POL = MASSTOR_SIL;
  if (POL_1 != 0 && POL_2 != 0) {
    ...
    if (SIGN == 0) {
      *POL = DIPC_DIPFMO(1, MASSTOR_LIST1(2));
      *BASE = MASSTOR_LIST1(DIPC_DIPFMO(1, TERM));
    } else {
      SIGN = SIGN + 1;
      if (SIGN == 2) {
        SIGN = 1;
      }
      *POL = DIPC_DIPFMO(1, MASSTOR_COMP(1, MASSTOR_LIST1(1)));
      *BASE = SACLIST_LEINST(MASSTOR_LIST1(DIPC_DIPFMO(1, TERM)), SIGN, DIPC_DIPFMO(1, TERM1));
    }
  }
  while (POL_3 != 0) {
    ...
    HM = DIPC_DIPFMO(HK, HT);
    ORBIT = GSYMFUIN_GINORP(PG, HM);
    POL_3 = DIPI_DIIPDF(POL_3, ORBIT);
    if (*BASE == MASSTOR_SIL) {
      *POL = DIPC_DIPFMO(HK, MASSTOR_LIST1(1));
      *BASE = MASSTOR_LIST1(DIPC_DIPFMO(1, HT));
    } else {
      N = MASSTOR_LENGTH(*BASE);
      POS = 0;
      for (;;) {
      ...
      POS = POS - 1;
      *BASE = SACLIST_LEINST(*BASE, POS, DIPC_DIPFMO(1, HT));
      NPOL = *POL;
      HT = MASSTOR_SIL;
      {
        LONGINT B_25 = 1, B_26 = N;
      ...
      *POL = DIPC_DIPFMO(HK, HT);
      while (NPOL != MASSTOR_SIL) {
        ...
        *POL = DIPI_DIIPSM(DIPC_DIPFMO(HK, HT), *POL);
      }
    }
  }
}
...
# endif
{
  ...
  *BASE_2 = MASSTOR_INV(XBASE);
  ...
  while (POL_1 != MASSTOR_SIL) {
    ...
    NPOL1 = DIPI_DIIPSM(DIPC_DIPFMO(HK1, HT1), NPOL1);
  }
  NPOL2 = 0;
  if (*POL_2 == 0) {
    *POL_2 = MASSTOR_SIL;
  ...
  while (*POL_2 != MASSTOR_SIL) {
    ...
    NPOL2 = DIPI_DIIPSM(DIPC_DIPFMO(HK2, HT2), NPOL2);
  }
  if (K == 0) {
    *POL_2 = DIPI_DIIPSM(NPOL1, NPOL2);
    return;
  }
  POL_1 = 0;
  if (NPOL1 == 0) {
  ...
  while (NPOL1 != MASSTOR_SIL) {
    ...
    POL_1 = DIPI_DIIPSM(POL_1, DIPC_DIPFMO(HK1, HT1));
  }
  *POL_2 = 0;
  if (NPOL2 == 0) {
    NPOL2 = MASSTOR_SIL;
  ...
  while (NPOL2 != MASSTOR_SIL) {
    ...
    *POL_2 = DIPI_DIIPSM(*POL_2, DIPI_DIIPPR(DIPI_DIIPEX(POL_1, EL), DIPC_DIPFMO(HK2, HT2)));
  }
}
...
# endif
{
  MASSTOR_LIST HK, HT, HM, RED_HT, BHM, BHT, PHM, PHK, PHT, TERM, XX, XHT, XHK, SIGMA, SIGMA_TL, BTL, NBTL, ORBIT_S, ORBIT_RED, OMEGA, PBASE, PPOL, SPG, PAIRS, MAT, NMAT, RS, ROW, NROW, SIGN, XLS, KOEFF, POS, DUMMY;
  ...
  *POL = 0;
  ...
  if (LOW != MINI || LOW != 0 && MAXI > 1) {
    ...
    ORBIT_S = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, SIGMA));
    ORBIT_RED = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, RED_HT));
    OMEGA = DIPI_DIIPDF(ORBIT, DIPI_DIIPPR(ORBIT_RED, ORBIT_S));
    EXTRACT(PG, ORBIT_RED, ORBIT_S, OMEGA, BASE, POL);
    return;
  }
  if (MAXI > 1) {
    {
      LONGINT B_33 = 1, B_34 = MAXI;
  ...
  *POL = MASSTOR_SIL;
  ...
  while (SIGMA_TL != MASSTOR_SIL) {
    ...
    if (DIPC_EVSIGN(RED_HT) > 0) {
      ORBIT_RED = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, RED_HT));
      XX = GSYMFUIN_GINORP(SPG, DIPC_DIPFMO(1, SIGMA));
    } else {
      XX = 0;
    }
    while (XX != 0) {
      DIPC_DIPMAD(XX, &XHK, &XHT, &DUMMY);
      XX = DIPI_DIIPDF(XX, GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, XHT)));
      ORBIT_S = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, XHT));
      EXTRACT(PG, ORBIT_RED, ORBIT_S, 0, &PBASE, &DUMMY);
      FLAG = FALSE;
      if (SACLIST_MEMBER(PBASE, PAIRS) != 1) {
        PAIRS = MASSTOR_COMP(PBASE, PAIRS);
        PPOL = DIPI_DIIPPR(ORBIT_RED, ORBIT_S);
        FLAG = TRUE;
      } else {
        PPOL = 0;
      }
      ...
      while (PPOL != 0) {
        ...
        PPOL = DIPI_DIIPDF(PPOL, GSYMFUIN_GINORP(PG, DIPC_DIPFMO(PHK, PHT)));
        ...
        I = I - 1;
        PBASE = SACLIST_LEINST(PBASE, I, DIPC_DIPFMO(1, PHT));
      ...
      while (NMAT != MASSTOR_SIL) {
        ...
        {
          LONGINT B_35 = 1, B_36 = MASSTOR_LENGTH(POS);
  ...
  if (MAT != MASSTOR_SIL) {
    ...
    {
      LONGINT B_37 = 1, B_38 = MASSTOR_LENGTH(MASSTOR_FIRST(MAT)) - 1;

      if (B_37 <= B_38)
        for (I = B_37;; I += 1) {
          RS = MASSTOR_COMP(0, RS);
          if (I >= B_38) break;
        }
    }
    RS = MASSTOR_COMP(1, RS);
    SACLDIO_LDSMKB(MAT, RS, &XLS, &DUMMY);
    ...
    while (XLS != MASSTOR_SIL) {
      ...
      if (XHK != 0) {
        if (MASSTOR_LENGTH(PBASE) == 1) {
          PPOL = DIPC_DIPFMO(1, MASSTOR_LIST1(2));
        } else {
          PPOL = DIPC_DIPFMO(1, MASSTOR_COMP(1, MASSTOR_LIST1(1)));
        }
        PPOL = DIPI_DIIPIP(PPOL, XHK);
        OMEGA = DIPI_DIIPDF(OMEGA, GSYMFUIN_GINCHK(PG, PBASE, PPOL));
        MERGE(0, PBASE, PPOL, BASE, POL);
      }
    }
    EXTRACT(PG, 0, 0, OMEGA, &PBASE, &PPOL);
    MERGE(0, PBASE, PPOL, BASE, POL);
    GSYMFUIN_GINCHKBAS(BASE, POL);
    return;
  }
  *BASE = MASSTOR_SIL;
  *POL = MASSTOR_SIL;
  ...
  while (SIGMA_TL != MASSTOR_SIL) {
    ...
    if (DIPC_EVSIGN(RED_HT) > 0) {
      ORBIT_S = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, SIGMA));
      XX = GSYMFUIN_GINORP(SPG, DIPC_DIPFMO(1, RED_HT));
    } else {
      XX = 0;
    }
    while (XX != 0) {
      DIPC_DIPMAD(XX, &XHK, &XHT, &DUMMY);
      XX = DIPI_DIIPDF(XX, GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, XHT)));
      ORBIT_RED = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, XHT));
      EXTRACT(PG, ORBIT_RED, ORBIT_S, 0, &PBASE, &DUMMY);
      FLAG = FALSE;
      if (SACLIST_MEMBER(PBASE, PAIRS) != 1) {
        PAIRS = MASSTOR_COMP(PBASE, PAIRS);
      ...
      while (NMAT != MASSTOR_SIL) {
        MASSTOR_ADV(NMAT, &NROW, &NMAT);
        {
          LONGINT B_39 = 1, B_40 = MASSTOR_LENGTH(POS);
  ...
  if (MAT != MASSTOR_SIL) {
    ...
    SACLIST_OWRITE(MASSTOR_LENGTH(MASSTOR_FIRST(MAT)));
    {
      LONGINT B_41 = 1, B_42 = MASSTOR_LENGTH(MASSTOR_FIRST(MAT)) - 1;
  ...
  *REM_POL = 0;
  ...
  while (POL != 0) {
    ...
    if (SACLIST_EQUAL(HT, MASSTOR_FIRST(ORBIT)) == 1) {
      ...
      POL = DIPI_DIIPDF(POL, HM);
      *REM_POL = DIPI_DIIPSM(*REM_POL, HM);
    }
  }
  I = 1;
  while (I <= MASSTOR_LENGTH(*BASE)) {
    MASBIOS_BLINES(0);
    MASBIOS_SWRITE("GINRED working... (BASE) ", 25L);
    SACLIST_OWRITE(*BASE);
  ...
  GSYMFUIN_GINCHKBAS(BASE, BASE_POL);
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("GINRED exit (BASE): ", 20L);
  SACLIST_OWRITE(*BASE);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GINRED exit (BASE_POL): ", 24L);
  SACLIST_OWRITE(*BASE_POL);
  MASBIOS_BLINES(0);
  MASBIOS_SWRITE("GINRED exit (REM_POL): ", 23L);
  SACLIST_OWRITE(*REM_POL);
}

MASSTOR_LIST GSYMFUIN_GSYADD
# ifdef __STDC__
(MASSTOR_LIST TERM)
# else
(TERM)
...
# endif
{
  MASSTOR_LIST ADDTERM, EL;
  MASELEM_GAMMAINT I, N, POS;

  ADDTERM = TERM;
  N = MASSTOR_LENGTH(TERM);
  if (N <= 2 && SACLIST_LELT(TERM, 1) == 2) {
    return ADDTERM;
  }
  if (N > 2 && SACLIST_LELT(TERM, 1) == N) {
    return ADDTERM;
  }
  POS = 1;
  EL = SACLIST_LELT(TERM, POS);
  {
    LONGINT B_43 = 2, B_44 = N;
  ...
  SACLIST_SLELT(ADDTERM, POS, EL + 1);
  if (POS < N) {
    {
      LONGINT B_45 = POS + 1, B_46 = N;

      if (B_45 <= B_46)
        for (I = B_45;; I += 1) {
          SACLIST_SLELT(ADDTERM, I, 0);
          if (I >= B_46) break;
        }
    }
  }
  ...
  for (;;) {
    ...
    if (SACLIST_LELT(ADDTERM, I) - SACLIST_LELT(ADDTERM, I + 1) > 1) {
      goto EXIT_12;
    }
    I = I + 1;
  } EXIT_12:;
  if (I == N && SACLIST_LELT(ADDTERM, N) == 0) {
    return ADDTERM;
  }
  if (SACLIST_LELT(ADDTERM, 1) == 1) {
    return ADDTERM;
  }
  return GSYMFUIN_GSYADD(ADDTERM);
}

MASSTOR_LIST GSYMFUIN_GINBAS
# ifdef __STDC__
(MASSTOR_LIST PG)
# else
(PG)
...
# endif
{
  ...
  for (;;) {
    ...
    while (ORBIT_SPG != 0) {
      ...
      ORBIT_PG = GSYMFUIN_GINORP(PG, DIPC_DIPFMO(1, HT));
      ORBIT_SPG = DIPI_DIIPDF(ORBIT_SPG, ORBIT_PG);
      REDUCE(PG, ORBIT_PG, &BASE, &POL);
      if (SACLIST_EQUAL(HT, MASSTOR_FIRST(MASSTOR_FIRST(BASE))) == 1) {
        NR2 = NR2 + 1;
        MASBIOS_BLINES(0);
        MASBIOS_SWRITE("GINBAS working... (Term ", 24L);
        SACLIST_OWRITE(NR1);
        MASBIOS_SWRITE("/", 1L);
        SACLIST_OWRITE(NR2);
        MASBIOS_SWRITE("): ", 3L);
        SACLIST_OWRITE(HT);
        XBASE = DIPI_DIIPSM(XBASE, DIPC_DIPFMO(1, HT));
      }
    }
  } EXIT_13:;
  BASE = MASSTOR_SIL;
  ...
  while (XBASE != MASSTOR_SIL) {
    ...
    BASE = MASSTOR_COMP(DIPC_DIPFMO(1, HT), BASE);
  }
  MASBIOS_BLINES(1);
  MASBIOS_SWRITE("GINBAS exit (BASE): ", 20L);
  SACLIST_OWRITE(BASE);
  MASBIOS_BLINES(0);
