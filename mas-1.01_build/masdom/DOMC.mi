(* ----------------------------------------------------------------------------
 * $Id: DOMC.mi,v 1.5 1994/09/06 11:48:51 rose Exp $
 * ----------------------------------------------------------------------------
 ...
 * ----------------------------------------------------------------------------
 * $Log: DOMC.mi,v $
 * Revision 1.5  1994/09/06  11:48:51  rose
 * modified comment
 *
 * Revision 1.4  1994/05/19  10:42:45  rose
 * Added DPNF, DPSP, DPSUGNF, DPSUGSP in connection with the new module DIPAGB
 *
 * Revision 1.3  1994/03/11  15:54:06  pesch
 * Minor corrections.
 *
 * Revision 1.2  1992/10/15  16:30:14  kredel
 * Changed rcsid variable
 *
 * Revision 1.1  1992/09/28  17:47:09  kredel
 * Initial revision
 *
 * ----------------------------------------------------------------------------
 *)

IMPLEMENTATION MODULE DOMC;

(* MAS Domain Complex Number Implementation Module. *)
...
FROM MASC  IMPORT CNREAD, CNWRITE, CSUM, CINT,  
                  CNEG, CNINV, CQ, CDIF, CPROD,
                  CDREAD, CDWRITE, CONE, CEXP;
...
CONST rcsidi = "$Id: DOMC.mi,v 1.5 1994/09/06 11:48:51 rose Exp $";
...
(*2*) (*write. *) IF SL < 0 THEN CNWRITE(AL) ELSE CDWRITE(AL,SL) END;
(*5*) RETURN; END DWRIT; 
...
VAR   d: Domain;
BEGIN
(*1*) d:=NewDom("C","Complex Number"); DOMCD:=d;
(*2*) SetDifFunc(d,DDIF); 
      SetExpFunc(d,DEXP); 
      SetFIntFunc(d,DFI); 
      SetFIPolFunc(d,DFIP); 
