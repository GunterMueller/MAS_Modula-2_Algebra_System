(* ----------------------------------------------------------------------------
 * $Id: DOMO.mi,v 1.5 1994/09/06 11:49:06 rose Exp $
 * ----------------------------------------------------------------------------
 ...
 * ----------------------------------------------------------------------------
 * $Log: DOMO.mi,v $
 * Revision 1.5  1994/09/06  11:49:06  rose
 * modified comment
 *
 * Revision 1.4  1994/05/19  10:43:02  rose
 * Added DPNF, DPSP, DPSUGNF, DPSUGSP in connection with the new module DIPAGB
 *
 * Revision 1.3  1994/03/11  15:54:08  pesch
 * Minor corrections.
 *
 * Revision 1.2  1992/10/15  16:30:19  kredel
 * Changed rcsid variable
 *
 * Revision 1.1  1992/09/28  17:47:12  kredel
 * Initial revision
 *
 * ----------------------------------------------------------------------------
 *)

IMPLEMENTATION MODULE DOMO;

(* MAS Domain Octonion Number Implementation Module. *)
...
FROM MASO  IMPORT ONREAD, ONWRITE, OSUM, OINT,  
                  ONEG, ONINV, OQ, ODIF, OPROD,
                  ODREAD, ODWRITE, OONE, OEXP;

FROM DIPAGB IMPORT EDIPSUGNOR, EDIPSUGSP;

FROM DIPGB IMPORT DIPNOR, DIPSP;

CONST rcsidi = "$Id: DOMO.mi,v 1.5 1994/09/06 11:49:06 rose Exp $";
...
(*2*) (*write. *) IF SL < 0 THEN ONWRITE(AL) ELSE ODWRITE(AL,SL) END;
(*5*) RETURN; END DWRIT; 
...
VAR   d: Domain;
BEGIN
(*1*) d:=NewDom("O","Octonion Number"); DOMOD:=d;
(*2*) SetDifFunc(d,DDIF); 
      SetExpFunc(d,DEXP); 
      SetFIntFunc(d,DFI); 
      SetFIPolFunc(d,DFIP); 
